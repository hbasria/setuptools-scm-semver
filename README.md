...
![image](https://travis-ci.org/pypa/setuptools_scm.svg?branch=master%0A%20:target:%20https://travis-ci.org/pypa/setuptools_scm)
...
Once configured, you can access the version number in your package via `pkg_resources` ([PEP-0396](https://www.python.org/dev/peps/pep-0396)). For example:
...
If using [setuptools 30.3.0](https://setuptools.readthedocs.io/en/latest/setuptools.html#configuring-setup-using-setup-cfg-files) or greater, you can store `setup_requires` configuration in `setup.cfg`. However, `use_scm_version` must still be placed in `setup.py`. For example:
...
You may also need to define a `pyproject.toml` file ([PEP-0518](https://www.python.org/dev/peps/pep-0518)) to ensure you have the required version of `setuptools`:
...
[setuptools\_scm\_git\_archive](https://pypi.python.org/pypi/setuptools_scm_git_archive)
...
> **note**
>
> Future versions of `setuptools_scm` will switch to [SemVer](http://semver.org/) by default hiding the the old behavior as an configurable option.
...
write\_to
A path to a file that gets replaced with a file containing the current version. It is ideal for creating a `version.py` file within the package, typically used to avoid using pkg\_resources.get\_distribution (which adds some overhead).

> **warning**
>
> Only files with .py and .txt extensions have builtin templates, for other file types it is necessary to provide write\_to\_template.
...
Defaults to the value of `setuptools_scm.config.DEFAULT_TAG_REGEX` (see [config.py](src/setuptools_scm/config.py)).

fallback\_version
A version string that will be used if no other method for detecting the version worked (e.g., when using a tarball with no metadata). If this is unset (the default), setuptools\_scm will error if it fails to detect the version.
...
Defaults to the value set by `setuptools_scm.git.DEFAULT_DESCRIBE` (see [git.py](src/setuptools_scm/git.py)).
...
SETUPTOOLS\_SCM\_PRETEND\_VERSION
...
Everyone interacting in the `setuptools_scm` project's codebases, issue trackers, chat rooms, and mailing lists is expected to follow the [PyPA Code of Conduct](https://www.pypa.io/en/latest/code-of-conduct/).
